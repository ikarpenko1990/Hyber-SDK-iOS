...
/**
 Easy `CoreData`.
 */
public final class HyberCoreDataHelper {
  ...
  private lazy var managedObjectModel: NSManagedObjectModel = {
    ...
    let bundle = Hyber.nonLocalizedBundle
  ...
  private lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator = {
    ...
    let url = self.applicationLibraryDirectory.URLByAppendingPathComponent(fileName + ".sqlite")
    ...
    do {
      ...
      hyberLog.error("Unresolved error \(wrappedError), \(wrappedError.userInfo)")
    }
...
internal extension HyberCoreDataHelper {
  
  /// `String` with .xcdatamodeld filename (without extension)
  private static var modelName: String = "HyberData"
   ...
   */
  internal static func initializeSharedInstance(withModelName modelName: String, fileName: String) {
    HyberCoreDataHelper.modelName = modelName
    HyberCoreDataHelper.fileName = fileName
  }
    ...
    managedObjectContext.performBlockAndWait {
      if self.managedObjectContext.hasChanges {
        do {
          ...
          hyberLog.error("Unresolved error \(nserror), \(nserror.userInfo)")
        }
      }
    }
  }
...
// MARK: - Static
extension HyberCoreDataHelper {
  
  /// The shared `HyberCoreDataHelper` object for the process. (read-only)
  private (set) static var sharedInstance: HyberCoreDataHelper = {
    return HyberCoreDataHelper(withModelName: modelName, fileName: fileName)
  }()
