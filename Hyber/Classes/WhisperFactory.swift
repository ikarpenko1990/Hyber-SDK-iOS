...
class WhisperFactory: NSObject {
  ...
  func craft(_ message: MessageNotification, navigationController: UINavigationController, action: WhisperAction) {
  ...
  func changeView(_ message: MessageNotification, action: WhisperAction) {
    presentTimer.invalidate()
    delayTimer.invalidate()
    hideView()
  ...
  func presentFired(_ timer: Timer) {
    ...
    let message = MessageNotification(title: title, textColor: textColor, backgroundColor: backgroundColor, images: images)
